                    ...
                    <tbody>
                        @forelse($students as $i => $s)
                            <tr style="background: white; transition: all 0.3s;">
                                ...
                                <td data-label="O'quvchi">
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        <img src="https://ui-avatars.com/api/?name={{ urlencode($s->name) }}&background=random"
                                            class="user-avatar" style="width: 45px; height: 45px; border-radius: 50%;">
                                ...
                                <td data-label="Telefon">{{ $s->phone ?? '-' }}</td>
                                <td data-label="O'rtacha">{{-- Baho --}}-</td>
                                <td data-label="Davomat">{{-- Davomat --}}-</td>
                                <td data-label="Status">
                                    @if ($s->status ?? false)
                                        <span
                                            style="background: #dcfce7; color: #059669; padding: 6px 14px; border-radius: 10px; font-size: 13px; font-weight: 700;">Faol</span>
                                    @else
                                        <span
                                            style="background: #fee2e2; color: #dc2626; padding: 6px 14px; border-radius: 10px; font-size: 13px; font-weight: 700;">NoFaol</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
